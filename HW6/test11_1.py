...
weights = np.array([0.3, 0.2, 0.5], dtype=float)
pd.DataFrame({"W": weights}).to_csv(base_dir / "test11_1_weights.csv", index=False)

returns = load_csv(base_dir / "test11_1_returns.csv", ref_dir / "test11_1_returns.csv")
returns.to_csv(base_dir / "test11_1_returns.csv", index=False)
weights = pd.read_csv(base_dir / "test11_1_weights.csv")["W"].to_numpy()
...
attribution = attribution.drop(columns=["Alpha"])
attribution.to_csv(base_dir / "testout11_1.csv", index=False)
